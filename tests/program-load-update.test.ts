...
it.each([['leg_press',70,100],['seated_leg_curl',40,50]] as const)(
 ...
 async (exerciseId, oldWeight, newWeight) => {
  ...
  try {
   ...
   const previous: WorkoutSession = {id:'previous',workoutType:'monday',status:'completed',startTimestamp:100,localDate:'2026-09-21',exerciseOrder:[exerciseId]};
   const current: WorkoutSession = {...previous,id:'current',status:'active',startTimestamp:200,localDate:'2026-09-28'};
   const record: SetRecord = {id:'old-set',sessionId:previous.id,workoutType:'monday',exerciseId,exerciseName:exerciseId,setNumber:1,actualWeight:oldWeight,weightUnit:'lb',weightKg:toKg(oldWeight,'lb'),actualReps:12,timestamp:110};
   ...
   expect((await db.exercises.toArray()).filter(e=>e.workoutType==='thursday')).toEqual(DEFAULT_EXERCISES.filter(e=>e.workoutType==='thursday').sort((a,b)=>a.id.localeCompare(b.id)));
