...
describe("backup export and import", () => {
  it("round-trips the entire database", async () => {
    ...
    expect(await target.exercises.count()).toBe(5);
    source.close();
    target.close();
  });
  ...
  it("rejects invalid data before modifying the database", async () => {
    ...
    expect(() => validateBackup({ format: "agym-workout-backup" })).toThrow();
    await expect(
      restoreBackup(database, { format: "agym-workout-backup" }),
    ).rejects.toThrow();
