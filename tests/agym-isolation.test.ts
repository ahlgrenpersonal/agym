...
describe("AGym schedule and independence", () => {
 ...
 it("provides exactly the agreed routine with no inherited starting loads", () => {
  ...
  expect(DEFAULT_EXERCISES.map(e=>e.defaultWeightLb)).toEqual([40,20,10,20,20]);
 ...
 it("service-worker activation deletes only obsolete AGym caches and ignores the original app's URLs",async () => {
  ...
  runInNewContext(readFileSync("public/sw.js","utf8"),{
   ...
   caches:{keys:async()=>["workout-shell-v23","unrelated-cache","agym-workout-shell-v0","agym-workout-shell-v4"],delete:async(key:string)=>{deleted.push(key);return true;}}
  });
